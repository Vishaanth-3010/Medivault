  ...
  return (
    <div className="mx-auto max-w-md rounded-xl border bg-white p-8 shadow-sm">
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        <select className="w-full rounded border px-3 py-2" value={role} onChange={(e) => setRole(e.target.value as "patient" | "doctor")}>
        ...
        <input className="w-full rounded border px-3 py-2" placeholder="Password (min 8 chars)" type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} required />
        {role === "doctor" && (
          <input className="w-full rounded border px-3 py-2" placeholder="Specialization" value={specialization} onChange={(e) => setSpecialization(e.target.value)} />
        )}
        {error && <p className="text-sm text-red-600">{error}</p>}
        <button className="w-full rounded bg-teal-700 py-2 text-white hover:bg-teal-800" type="submit">Create account</button>
      </form>
